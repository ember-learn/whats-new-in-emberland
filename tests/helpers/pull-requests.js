...
function setupCustomAssertions(assert) {
  assert.isPullRequestCorrect = (pullRequest, expectedValues) => {
    ...
    assert.dom('[data-test-field="Title"]', pullRequest)
      .hasAttribute(
        'href',
        title.href,
    ...
    assert.dom('[data-test-field="Author"]', pullRequest)
      .hasAttribute(
        'href',
        author.href,
    ...
    assert.dom('[data-test-field="Repo"]', pullRequest)
      .hasAttribute(
        'href',
        repo.href,
      ...
      .hasText(
        repo.text,
        `We see the correct text for the pull request repo.`
      );
  };
}
